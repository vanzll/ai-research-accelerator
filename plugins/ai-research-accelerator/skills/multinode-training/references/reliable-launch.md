...
an idle GPU holder. Gate the child from GPU or rendezvous work until its active
PGID is durably recorded in the generation-fenced lease; a dead supervisor with
live group members must quarantine the lease and block a new attempt until
verified drain. Exercise normal exit, signals, zombie
